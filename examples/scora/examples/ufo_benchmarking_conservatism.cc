...
void DoMain() {
    ...
    std::vector<float> risk_budgets{0.4, 0.3, 0.2, 0.17, 0.15, 0.13, 0.1, 0.08, 0.05, 0.04, 0.02, 0.01, 0.008, 0.007, 0.006, 0.005, 0.003, 0.002, 0.001, 0.0007, 0.0005, 0.0001};
    std::vector<float> risk_estimates;
    std::vector<float> true_risks;

    std::cout << "obstacle_cov,state_cov,risk_budget,risk_estimate,true_risk" << std::endl;
    ...
    for (float risk_budget : risk_budgets) {
        ...
        double obstacle_cov = 0.01;
        Eigen::Matrix3d uncertain_obstacle_covariance;
        uncertain_obstacle_covariance << obstacle_cov, 0.0, 0.0,
                                         0.0, obstacle_cov, 0.0,
                                         0.0, 0.0, obstacle_cov;
        ...
        double state_cov = 0.005;
        Eigen::Matrix3d state_covariance;
        state_covariance << state_cov, 0.0, 0.0,
                            0.0, state_cov, 0.0,
                            0.0, 0.0, state_cov;
        ...
        // prog->AddLinearCost(risk_allocations(0) + risk_allocations(1));
        ...
        drake::log()->warn("Risk budget {}", risk_budget);
        ...
        // std::cout << "obstacle_cov,state_cov,risk_budget,risk_estimate,true_risk" << std::endl;
        std::cout << obstacle_cov << "," << state_cov << "," << risk_budget << "," << estimated_risk << "," << true_risk << std::endl;
    }

    // drake::log()->warn("Budget, estimate, true");
    // for (int i = 0; i < risk_estimates.size(); i++) {
    //     drake::log()->warn("{}, {}, {}", risk_budgets[i], risk_estimates[i], true_risks[i]);
    // }
}
