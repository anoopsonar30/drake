...
void load_plant_components(MultibodyPlant<double>& plant, bool fix_obstacles) {
    ...
    std::string obstacle_course_model_file = "./examples/models/tunnel.urdf";
    ...
    if (fix_obstacles) {
        ...
        drake::Vector3<double> obstacle_1_pos{-0.5, 0.0, 1.0};
        ...
        drake::Vector3<double> obstacle_2_pos{0.5, 0.0, 1.0};
...
void DoMain() {
    ...
    // std::vector<float> risk_budgets{FLAGS_delta};
    std::vector<float> risk_budgets{0.3, 0.25, 0.2, 0.1};
    ...
    for (float risk_budget : risk_budgets) {
        ...
        // Seed the chance-constrained solver with the linear interpolation
        guess = VectorX<double>::Zero(prog->num_vars());
        for (int t = 0; t < T; t++) {
            Eigen::VectorXd q_values = trajectory.value(t * timestep);
            prog->SetDecisionVariableValueInVector(
        ...
        drake::log()->info("======================================================================================");
        drake::log()->warn("Solved {} chance-constrained optimization problems, avg. duration {} ms",
        ...
        true_risks.push_back(validate_result(result, q, joint_chance_constraint));
        ...
        times.push_back(double(duration.count()) / FLAGS_num_benchmark_runs);
    }
