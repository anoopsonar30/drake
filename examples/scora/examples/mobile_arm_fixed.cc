...
void DoMain() {
    ...
    double fixed_allocation = FLAGS_delta / FLAGS_T;
    for (int t = 0; t < FLAGS_T; t++) {
        ...
        joint_prog->AddConstraint(risk_allocations(0) + risk_allocations(1) <= fixed_allocation);
        joint_prog->AddConstraint(risk_allocations(0) >= 0.0);
        joint_prog->AddConstraint(risk_allocations(1) >= 0.0);
        // drake::log()->warn("Add risk allocs");
        ...
        drake::solvers::VectorXDecisionVariable all_decision_variables(num_positions + 2);
        // drake::log()->warn("{}", all_decision_variables);
        // drake::log()->warn("{}", q.row(t));
        // drake::log()->warn("{}", risk_allocations);
        all_decision_variables << q.row(t).transpose(), risk_allocations;
        // drake::log()->warn("vectorize");
    ...
    for (int i = 0; i < FLAGS_num_benchmark_runs; i++) {
        // std::unique_ptr<drake::solvers::MathematicalProgram> prog_copy = joint_prog->Clone();
        drake::solvers::MathematicalProgramResult joint_result = solver.Solve(*joint_prog, guess);
    }
    ...
    drake::log()->warn("Optimal cost {}",
                            double(joint_result.get_optimal_cost()));
    ...
    drake::log()->warn("\t fixed risk allocation:");
    auto joint_chance_constraint = std::make_shared<ccopt::JointCollisionChanceConstraint>(
        &plant, plant_context, world_manager,
        risk_precision,
        FLAGS_T,
